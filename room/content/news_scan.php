...
<?php
$left = @mysql_query ( "select * from tb_news where id> $sid order by id asc limit 0,1" );
...
if (! empty ( $left_row ['id'] )) {
	?>
上一篇：<a href="news_scan.php?sid=<?php echo $left_row['id'];?>" title="<?=$left_row['title']?>">
...
<?php
$right = @mysql_query ( "select * from tb_news where id < $sid order by id desc limit 0,1" );
...
if (! empty ( $right_row ['id'] )) {
	?>
下一篇：<a href="news_scan.php?sid=<?php echo $right_row['id'];?>" title="<?=$right_row['title']?>">
